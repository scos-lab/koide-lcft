...
We observe that the complete Koide formula for charged lepton masses can be expressed in terms of two integers: the central charge c = -2 of the logarithmic conformal field theory of symplectic fermions and the orbifold order N = 3. The Koide angle δ₀ = |c|/N² = 2/9 equals the Z₃ twist field dimension (Kausch 2000, h = -λ(1-λ)/2 at λ = 1/3). The Koide K-value K = |c|/N = 2/3 fixes the amplitude ratio √2. The use of √m (rather than m) follows from interpreting √m as the CFT transition amplitude, with mass = amplitude² (Born rule). We conjecture that this structure reflects an underlying c = -2 LCFT in the lepton sector. If correct, charged lepton mass ratios are determined with zero free parameters: m_μ/m_e = 206.770 (experiment: 206.768, deviation 0.001%) and m_τ = 1776.97 ± 0.01 MeV (HFLAV 2025: 1776.96 ± 0.09 MeV, within 0.1σ). No prior connection between the Koide formula and conformal field theory has been reported.
...
**Why √m?** The Koide formula acts on √m, not m. In quantum mechanics, observables are quadratic in amplitudes (Born rule): probability ~ |amplitude|². If √m_k is the CFT transition amplitude A_k for the k-th sector, then m_k = |A_k|². The Fourier decomposition of A_k on Z₃ is a linear operation on a linear quantity — exactly as expected from quantum mechanical superposition. The mass, being amplitude-squared, inherits the Koide structure. This interpretation identifies M with the overall amplitude scale A₀, and the Koide formula becomes: A_k = A₀(1 + √2·cos(2πk/3 + 2/9)), m_k = A_k².

**Summary.** We have observed that the complete Koide formula can be expressed in terms of two integers: the central charge c = -2 and the orbifold order N = 3. The Koide angle is δ₀ = |c|/N² = 2/9 (the Z₃ twist field dimension), the Koide K-value is K = |c|/N = 2/3, and √m is interpreted as the CFT transition amplitude (with mass = amplitude² via the Born rule). We conjecture that this structure reflects an underlying c = -2 LCFT Z₃ orbifold in the lepton sector. If the conjecture holds, it yields charged lepton mass ratios with zero adjustable parameters and a τ mass prediction m_τ = 1776.97 MeV, in excellent agreement with the 2025 HFLAV world average (1776.96 ± 0.09 MeV, 0.1σ). The conjecture is testable at Belle II.
